...
from agent.tools.count_words_in_file_tool import count_words_in_file_tool
...
class PDFToolsTest(unittest.TestCase):
    def test_extract_text_from_pdf(self):
        text = extract_text_from_pdf_tool("tests/sample.pdf")
    ...
    def test_summarize_pdf(self):
        summary = summarize_pdf_tool("tests/sample.pdf", sentences=1)
    ...
    def test_count_words_in_file(self):
        with tempfile.NamedTemporaryFile("w+", delete=False) as f:
            ...
            path = f.name
        try:
            count = count_words_in_file_tool(path)
